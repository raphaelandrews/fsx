...
import { circuitsQueryOptions } from "~/db/queries";
...
import type {
  ...
  CircuitClub,
  CircuitPhase,
} from "~/components/circuits/types";

import { categories } from "~/components/circuits/data/data";
import CategoryFilter from "~/components/circuits/category-filter";
import {
  circuitClubsColumns,
  circuitSchoolSubcomponentColumns,
  circuitTotalColumns,
} from "~/components/circuits/columns";
import { DataTable } from "~/components/circuits/data-table";
import { DataTableSchool } from "~/components/circuits/data-table-school";
...
export const Route = createFileRoute("/_default/circuitos/")({
  loader: async ({ context: { queryClient } }) => {
    await queryClient.ensureQueryData(circuitsQueryOptions());
  },
  head: () => ({
    meta: [
      {
...
function RouteComponent() {
  const { data, error } = useQuery(circuitsQueryOptions());

  if (error) {
    return <ErrorComponent error={error} />;
  }

  const circuits = data ? (data as Circuit[]) : [];

  const [circuitPodiums, setCircuitPodiums] = React.useState<CircuitPodium[]>(
    []
  );
  const [selectedPhase, setSelectedPhase] = React.useState<string | undefined>(
    undefined
  );
