...
const fetchAnnouncementById = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data: id }: { data: number }) => {
    try {
      console.info(`Fetching announcement id=${id} from:`, API_BASE_URL);

      const resp = await axios.get(`${API_BASE_URL}/announcement/${id}`);
...
export function announcementByIdQueryOptions(id: number) {
  return queryOptions({
