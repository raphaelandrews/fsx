...
export const fetchAnnouncementsByPage = createServerFn({ method: 'GET' })
  ...
  .handler(async (ctx) => {
    ...
    try {
      ...
      const parsed = APIAnnouncementsResponseSchema.safeParse(response.data);
      ...
      const message = error instanceof Error ? error.message : `Failed to fetch announcements page ${page}`;
      throw new Error(message);
    }
  });

export const announcementsByPageQueryOptions = (page = 1) =>
  queryOptions({
    queryKey: ['announcements', page] as const,
    queryFn: () => fetchAnnouncementsByPage({ data: page }),
