...
export const Route = createFileRoute("/_auth/dashboard/school-results/")({
  head: () => ({ meta: [{ title: "School Results - Admin - FSX" }] }),
  loader: ({ context }) => context.queryClient.ensureQueryData(context.trpc.schoolResults.list.queryOptions()),
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation({
    ...trpc.schoolResults.delete.mutationOptions(),
    onSuccess: () => { qc.invalidateQueries(trpc.schoolResults.list.queryFilter()); toast.success("Result deleted"); },
    onError: () => toast.error("Failed to delete result"),
  });

  const deleteAllMutation = useMutation({
    ...trpc.schoolResults.deleteAll.mutationOptions(),
    onSuccess: () => { qc.invalidateQueries(trpc.schoolResults.list.queryFilter()); toast.success("All results deleted"); },
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h1 className="font-bold text-2xl">School Results</h1>
              ...
              <TableRow key={r.id}>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-1">
                    <Link to="/dashboard/school-results/$id" params={{ id: String(r.id) }}><Button size="sm" variant="outline">Edit</Button></Link>
