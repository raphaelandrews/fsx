  ...
  const form = useForm({
    ...
    onSubmit: ({ value }) => {
      updateMutation.mutate({
        ...
        imageUrl: value.imageUrl || undefined, content: value.content, published: value.published,
      });
    },
  });
  ...
  return (
    <div className="mx-auto max-w-lg">
      ...
      <form onSubmit={(e) => { e.preventDefault(); form.handleSubmit(); }} className="space-y-4">
        ...
        <form.Field name="imageUrl">{(f) => (<div className="space-y-2"><Label htmlFor={f.name}>Image URL</Label><Input id={f.name} value={f.state.value} onBlur={f.handleBlur} onChange={(e) => f.handleChange(e.target.value)} /></div>)}</form.Field>
