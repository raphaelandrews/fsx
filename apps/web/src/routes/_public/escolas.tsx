...
const AGE_GROUPS = ["8", "10", "12", "14", "16", "18"] as const;
type AgeGroup = (typeof AGE_GROUPS)[number];
type Sex = "male" | "female";
type Modality = "individual" | "team";

const INDIVIDUAL_MEDAL_WEIGHT = 1;
const TEAM_MEDAL_WEIGHT = 2;
...
const SUB_SCOPE_TABS: { id: SubScopeId; label: string }[] = [
...
function subScopeToFilters(
  subScope: SubScopeId
): { sex?: Sex; modality?: Modality } {
  switch (subScope) {
    case "geral":
      return {};
...
export const Route = createFileRoute("/_public/escolas")({
  ...
  loader: ({ context, deps }) => {
    ...
    const filters = deps.idade
      ? subScopeToFilters((deps.escopo ?? "geral") as SubScopeId)
      : {};
...
function RouteComponent() {
  ...
  const filters = search.idade
    ? subScopeToFilters((search.escopo ?? "geral") as SubScopeId)
    : {};
  ...
  const { data: allResults = [] } = useSuspenseQuery(
    trpc.schoolResults.list.queryOptions()
  );
      ...
      >
        <DialogContent className="max-w-2xl">
          ...
          {clubResults.length === 0 ? (
            <p className="text-muted-foreground text-sm">
              Nenhum resultado neste recorte.
            </p>
          ) : (
...
function ResultRow({ result }: { result: SchoolResult }) {
  ...
  const medalCount =
    result.modality === "team" ? TEAM_MEDAL_WEIGHT : INDIVIDUAL_MEDAL_WEIGHT;

  const medalVariant =
    place === 1 ? "default" : place === 2 ? "secondary" : "outline";

  return (
    <li className="flex flex-col gap-1 px-4 py-3">
      <div className="flex items-center justify-between gap-3">
        <span className="text-sm font-medium">
          {AGE_LABEL[result.ageGroup as AgeGroup]} ·{" "}
          {SEX_LABEL[result.sex as Sex]} ·{" "}
