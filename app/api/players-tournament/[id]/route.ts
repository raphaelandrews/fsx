...
export async function PUT(
    ...
    async function getMaxId(table: typeof players | typeof playersToTournaments): Promise<number | null> {
        try {
    ...
    async function getNewId(table: typeof players | typeof playersToTournaments): Promise<number | null> {
        try {
    ...
    if (!ratingTypeEnum.enumValues.includes(ratingType)) {
        return new NextResponse(`Invalid rating type. Must be one of: ${ratingTypeEnum.enumValues.join(', ')}`, { status: 400 });
    }
    ...
    try {
        const result = await db.transaction(async (tx) => {
            ...
            if (oldRating === undefined || oldRating === null) {
                tx.rollback();
                return new NextResponse(`Rating type '${ratingType}' not found for player.`, { status: 404 });
            }

            const playerToTournamentId = await getNewId(playersToTournaments);
            if (playerToTournamentId === null) {
                tx.rollback();
                return new NextResponse('Failed to generate player-tournament relation ID.', { status: 500 });
            }
            ...
            const playerUpdateData: Partial<typeof players.$inferInsert> = {
                ...
                ...(locationId !== undefined && currentPlayer.locationId === null && { locationId }),
        ...
        return new NextResponse('Internal server error during player update.', { status: 500 });
    }
}
