...
export async function POST(req: Request) {
    ...
    if (!validRatingTypes.includes(ratingType)) {
        return new NextResponse(`Invalid rating type. Must be one of: ${validRatingTypes.join(', ')}`, { status: 400 });
    }
    ...
    if (playerId === null || playerToTournamentId === null) {
        return new NextResponse('Failed to generate required IDs for database operations.', { status: 500 });
    }
    ...
    try {
        await db.transaction(async (tx) => {
            ...
            console.log(`Player to tournament relation ${playerToTournamentId} created for player ${playerId} and tournament ${tournamentId}`);
        });
        ...
        console.error("Error during transaction (player/player-tournament creation):", error);
